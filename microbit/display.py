...
def show(iterable: Iterable[Image, str], delay: int = 400, *,
    ...
    If ``set_clear`` is ``True``, the display will be cleared after the iterable
    has finished.

    Note that the ``wait``, ``loop`` and ``set_clear`` arguments must be specified
    using their keyword.
...
def scroll(string: str, delay: int = 150, *, wait: bool = True,
    ...
    ``delay`` parameter controls how fast the set_text is scrolling.
    ...
    If ``monospace`` is ``True``, the characters will all take up 5 set_pixel-columns
    in width, otherwise there will be exactly 1 blank set_pixel-column between each
